...
Every single well-known "real-time" wrapper for Node.js, every single one, does in fact add a significant CPU overhead to any networking performed. Often times the overhead can be 10x or even 100x, meaning your "super efficient cluster built with µWS" has now deteriorated into a bad joke. There's no such thing as efficiency served through old and misdesigned wrapper scripts.
